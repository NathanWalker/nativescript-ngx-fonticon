...
const getCollectionName = function(className: string, args: any[]): string {
...
export class TNSFontIconPipe implements PipeTransform, OnDestroy {
  ...
  transform(className: string, args?: any[]) {
    ...
    if (
      !this._value ||
      (this.fonticon.css &&
        this.fonticon.css[this._collectionName] &&
      ...
      this._iconSub = this.fonticon.filesLoaded.subscribe((data: any) => {
        if (
          data &&
          data[this._collectionName] &&
          data[this._collectionName][className]
        ) {
